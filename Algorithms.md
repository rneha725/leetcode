...
#### Understanding the complexity of BFS and DFS
So, we know that BFS creates a queue, and we use a while loop and keep on popping elements till it is empty. At the same time we push the unvisited nodes in the queue. To understand the complexity, understand this, we always visit the unvisited edges, For while loop, the complexity for visiting each node will loop like this

```
1. V1: 1 + E1 (unvisited edges)
2. V2: 1 + E2 (unvisited edges)
... repeated V times

When we add all these we get: 1*V + (E1 + E2 + E3...)
Here E1 + E2 + E3 ... = E

so we get comlexity as O(V + E)
```
